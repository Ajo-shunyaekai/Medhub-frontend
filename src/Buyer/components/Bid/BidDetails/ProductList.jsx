...
const ProductList = ({ socket }) => {
  ...
  const handleRequestQuote = async (row) => {
    try {
      ...
      await postRequestWithToken(
        `bid/send-enquiry/${obj.bidId}/${obj.itemId}/${obj.participantId}`,
        obj,
        async (response) => {
          if (response.code === 200) {
  ...
  useEffect(() => {
    ...
    (bidDetails?.additionalDetails || []).forEach((item) => {
      const favourites =
        item.participants?.filter((mem) => !mem?.favourite) || [];
      const nonFavourites =
        item.participants?.filter((mem) => mem?.favourite) || [];

      const combinedParticipants = [...favourites, ...nonFavourites];
    ...
    // Sort again to double-confirm favourites on top (optional)
    const sortedRows = allRows.sort((a, b) => {
      if (a.favourite === b.favourite) return;
