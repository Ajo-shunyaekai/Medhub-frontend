...
const useFileUpload = (
  ...
  const [filesMerged2, setFilesMerged2] = useState(
    initialValues?.categoryDetailsFile || []
  );
  ...
  const onDrop = useCallback(
    (acceptedFiles) => {
      ...
      const updatedcategoryDetailsFiles = [
        ...(initialValues?.categoryDetailsFile || []), // Use `values` instead of `initialValues`
        ...acceptedFiles, // Add all the accepted files
      ];

      // Update Formik's field value with the new files
      // setFieldValue("categoryDetailsFile", updatedcategoryDetailsFiles);
    },
  ...
  // Effect to handle initial file state
  useEffect(() => {
    if (initialValues?.categoryDetailsFile?.length > 0) {
      setFilesMerged2(initialValues?.categoryDetailsFile); // Set the existing files
    }
  }, [initialValues?.categoryDetailsFile]);
  ...
  const removeFile = (index, event) => {
    ...
    // setFieldValue("categoryDetailsFile", initialValues?.categoryDetailsFile?.filter((_, i) => i !== fileIndex)); // Update Formik field with the new files
  };
...
// AddProductAddOtherDetailsFileUpload Component
const AddProductAddOtherDetailsFileUpload = ({
  setFieldValue,
  fieldInputName,
  initialValues,
  label,
...
export default AddProductAddOtherDetailsFileUpload;
