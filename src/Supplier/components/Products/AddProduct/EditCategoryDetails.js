...
const useFileUpload = (
  ...
  const onDrop = useCallback(
    (acceptedFiles) => {
      ...
      const updatedcategoryDetailsFiles = [
        ...(initialValues?.categoryDetailsFileNew || []),
        ...acceptedFiles,
      ];
      setFieldValue("categoryDetailsFileNew", updatedcategoryDetailsFiles);
      setFilesNew((prev) => {
  ...
  const removeFile = (index, event, arrayToFilter, file) => {
    ...
    if (arrayToFilter === "new") {
      ...
      setFilesNew([]);
      const indexToRemove = initialValues?.categoryDetailsFile?.findIndex(
        (ele, index) => JSON.stringify(ele) === file
      );
      const filteredValues = initialValues?.categoryDetailsFile?.filter(
      ...
      setFilesNew([]);
      const indexToRemove = initialValues?.categoryDetailsFileNew?.findIndex(
        (ele, index) => JSON.stringify(ele) === file
      );
